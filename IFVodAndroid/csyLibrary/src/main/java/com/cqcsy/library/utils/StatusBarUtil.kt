...
// decorView 中添加一个与状态栏大小的 view
     ...
     * @return
     */
    private fun createStatusBarDrawableView(
        activity: Activity,
        drawable: Drawable
    ): View { // 绘制一个和状态栏一样高的矩形
...
// 添加 statusBarView 到布局中
    ...
    fun clearTransparentForWindow(activity: Activity) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            activity.window
    ...
    private fun hasNotchHw(activity: Activity): Boolean {
        return try {
            ...
            val size = cl.loadClass("com.huawei.android.util.HwNotchSizeUtil")
            val get = size.getMethod("hasNotchInScreen")
            get.invoke(size) as Boolean
        } catch (e: Exception) {
            false
        }
    }
