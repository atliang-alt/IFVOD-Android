 ...
 */
package com.cqcsy.library.download.server;

import android.content.ContentValues;
import android.text.TextUtils;

import com.cqcsy.library.download.task.PriorityRunnable;
...
public class DownloadTask implements Runnable {
    ...
    public void run() {
        ...
        if (progress.totalSize == -1) {
            // content-range: bytes 0-1343381036/1343381037
            String range = response.header("content-range");
            if (body.contentLength() != -1) {
                progress.totalSize = body.contentLength();
            } else if (!TextUtils.isEmpty(range) && range.contains("/")) {
                String size = range.substring(range.indexOf("/") + 1);
                progress.totalSize = Long.parseLong(size);
            }
        }
