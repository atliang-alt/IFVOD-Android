...
import kotlinx.android.synthetic.main.layout_video_bullet_input.*
import org.json.JSONObject

/**
 * 发布弹幕
 */
class DanamaInputDialog(context: Context) : BottomBaseDialog(context) {
    val MAX_DUMAKU = 25
    var listener: OnSendDanamaListener? = null
    var selectColor: Int? = null
    var itemList: ArrayList<ColorItem> = ArrayList()

    // 0 所有 1 昵称和头像 2 国家 3 全部关闭
    private var prefix = 3

    interface OnSendDanamaListener {
        fun onSend(input: String)
        fun onOpenVip()
    }

    fun setOnSendDanamaListener(listener: OnSendDanamaListener) {
        this.listener = listener
    }

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.layout_video_bullet_input)
        ...
        ClickUtils.applyGlobalDebouncing(headerNick) {
            if (!GlobalValue.isVipUser()) {
                listener?.onOpenVip()
        ...
        ClickUtils.applyGlobalDebouncing(location) {
            if (!GlobalValue.isVipUser()) {
                listener?.onOpenVip()
                return@applyGlobalDebouncing
            }
        ...
        if (!GlobalValue.isVipUser()) {
            SPUtils.getInstance().put(Constant.KEY_SEND_DANAMA_POSITION + GlobalValue.userInfoBean?.id, 0)
        }
    ...
    private fun getBarrageConfig() {
        HttpRequest.post(RequestUrls.GET_BARRAGE_CONFIG, object : HttpCallBack<JSONObject>() {
    ...
    fun isSetting(): Boolean {
        return positionContent.visibility == View.VISIBLE
    }
    ...
    private fun initColor() {
        ...
        val color = SPUtils.getInstance().getString(Constant.KEY_SEND_DANAMA_COLOR + GlobalValue.userInfoBean?.id, "")
    ...
    private fun setColor() {
        colorList.layoutManager = GridLayoutManager(context, 4)
        colorList.addItemDecoration(XGridBuilder(context).setHLineSpacing(20f).setVLineSpacing(10f).setIncludeEdge(true).build())
        val adapter = object : BaseQuickAdapter<ColorItem, BaseViewHolder>(R.layout.layout_color_item, itemList) {
